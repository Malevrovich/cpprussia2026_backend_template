...
### 2. Сборка и проверка example_service

Перед реализацией собственных сервисов рекомендуется убедиться, что окружение работает корректно. Для этого в проекте уже есть готовый сервис `example_service`, который можно собрать и протестировать.

1. **Соберите example_service:**
   ```bash
   cd backend/example_service
   make build-release
   ```
   Бинарник появится в `backend/example_service/build-release/example_service`.
...
2. **Запустите Docker Compose:**
   ...
   ```
   Сервис `example-service` уже раскомментирован в `docker-compose.yml` и будет запущен на порту 8009.

3. **Проверьте, что сервис работает:**
   ```bash
   curl "http://localhost:8009/hello?name=World"
   ```
   Ожидаемый ответ: `"Hello, World!\n"`

После успешной проверки можно приступать к реализации собственных микросервисов, следуя инструкциям ниже.
...
Кроме того, в `auth_service` вам не обязательно сразу реализовывать полноценную генерацию JWT‑токенов. Для начала достаточно, чтобы сервис возвращал какой‑либо токен (например, случайную строку). После реализации основной части задания вы можете вернуться и доделать полноценную авторизацию с подписанными JWT‑токенами, в качестве доп задания.
